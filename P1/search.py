...
def graph_search(problem, fringe):
    ...
    closed = {}
    fringe.append(Node(problem.initial))
    # Counter -> numero de nodos expandidos / +1 tab nodos visitados
    counter = 0
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            print "\n Nodos expandidos: " + str(counter)
            return node
...
def search_ramification(problem):
...
def search_heuristic(problem):
    """Este metodo ordena por el coste total acumulado y una heuristica"""
    return graph_search(problem, ramificationUnderestimation(problem))
